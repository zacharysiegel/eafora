...
readonly SHELL_PATH="${SITE_DIR}/index.html"
...
if [[ ! -f "$SHELL_PATH" ]]; then
    fail "${SHELL_PATH} is missing, so the deploy would serve no document at /; run ./scripts/build/build-site.sh"
fi
...
while IFS= read -r referenced_path; do
    ...
    if ! printf '%s' "$referenced_path" | grep -Eq "$HASHED_NAME_PATTERN"; then
        fail "the shell references ${referenced_path}, whose name carries no content hash, but _headers serves /pkg/* as immutable; build with ./scripts/build/build-site.sh"
    fi

    reference_count=$((reference_count + 1))
done < <(grep -o '/pkg/[A-Za-z0-9._-]*' "$SHELL_PATH" | sort -u)
