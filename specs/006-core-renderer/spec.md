...
- The choropleth color function (FR-016) computes the per-country normalized value once per `draw_frame` from the bundle's SQLite shard **bytes**. 005-core-data's `Bundle` carries raw shard bytes (`shard_bytes: BTreeMap<StatisticShardKey, Vec<u8>>`), NOT an open SQLite connection — `Bundle::open` opens no connection and does not cache per-statistic `min`/`max`. This feature opens its own connection against `bundle.shard_bytes` (see the deferral note below) and computes/caches the value range itself (in the Renderer or a per-statistic value-map at first draw).
- **Deferred from 005: the cross-platform SQLite `Connection` bridge.** Originally scoped as 005's `shared::sqlite::vfs` (typedef `Connection = rusqlite::Connection` / `= sqlite_wasm_rs::Connection`), it was descoped from 005 and lands HERE in 006, in `shared::sqlite`, because (a) its API surface is defined by the renderer's queries (single-value lookups, multi-shard `ATTACH` + `UNION`, year-scrubber period queries), and (b) the wasm backend `sqlite-wasm-rs` (0.4.x) exposes only raw libsqlite3 C bindings — there is NO `Connection` type — so the bridge must be a real wrapper, not a typedef: native delegates to `rusqlite` (enable its `serialize` feature for `Connection::deserialize` of the in-memory bytes); wasm32 wraps the raw `sqlite3_*` FFI (open + `sqlite3_deserialize` + prepare/step/column) behind the same facade, verified by a headless-browser `wasm-bindgen-test`. 005 already ships the stable half — `shared::sqlite::schema` (the `application_id`/`user_version` magic numbers, table/column-name constants, `shard_schema_ddl()`, `validate_shard_header()`) — plus the shard bytes on the `Bundle`. The renderer opens its own thread-local connection from those bytes and `ATTACH`es the authorized shards.
