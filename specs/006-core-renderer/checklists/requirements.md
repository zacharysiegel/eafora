...
> §Scope cutoff explicitly names what's NOT in this feature (per-platform FFI modules, `hover_scale` easing curves, GPU-based label rendering). Each cut item is paired with where it lives instead (003 / 004, v2+ animation work, future SDF / MSDF pipeline). The zoom-to-country transition (`ViewportTransition`), formerly cut to v1.5+, has since landed in `shared::map::viewport_transition` with the driver wiring in the web client (003).
