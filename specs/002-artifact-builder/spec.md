...
After the weekly WB WDI ingestion run completes on the Mac mini, the launchd-managed schedule chains a build + publish: every value with `superseded is null` in `statistic_value` is rolled into the source-preference merge, emitted into SQLite + FlatGeobuf shards with content-hashed filenames, written to manifest.json, **delivered to the configured destination** (local disk OR Cloudflare R2 — toggleable per-invocation), and recorded in `artifact_version` with a destination-appropriate URL. **Pre-product-ship**: the periodic job uses the local-disk destination — artifacts accumulate on the Mac mini, R2 is not touched, no public-facing CDN traffic. **Post-ship**: the same job switches to the R2 destination by changing one flag in the launchd plist.
...
2. **Given** the same state but `publish --destination=r2` is invoked, **When** publish runs against a real R2 bucket with valid credentials, **Then** each shard + manifest is uploaded under its content-hashed key with the per-file `Cache-Control` headers; `artifact_version.manifest_url` resolves to the CDN URL; the destination switch from `local` to `r2` requires no source-code change — only the flag.
...
When publish runs, either the entire artifact set reaches the configured destination AND the `artifact_version` row is inserted, or nothing is recorded. There's no half-state where `artifact_version` claims a publication that the destination can't actually serve. The invariant generalizes from the architecture's R2-only framing: **`artifact_version` rows mean "fetchable from the destination recorded in `manifest_url`"** — `file://` for local destination, `https://...cdn...` for R2.

**Acceptance Scenarios**:

1. **Given** an R2 PUT fails partway through publish (network blip, 5xx from R2), **When** publish returns the error, **Then** `artifact_version` has no row for that version_label, AND the local build remains on disk so the developer can retry without re-running `build`. Any shards that succeeded upload before the failure remain in R2 — they're idempotently overwritten on retry (content-hashed key + same bytes = no-op PUT).
...
3. **Given** every shard + manifest reaches the destination successfully, **When** `insert_artifact_version` then fails (DB connection drops, unique-constraint violation on `version_label`), **Then** the operator gets a clear AppError naming what was published but not recorded; manual recovery is re-running `publish` with the same version_label (which detects the existing destination files via idempotent re-publish and either re-attempts the insert or surfaces the conflict).
...
- **FR-009**: For the R2 store impl, system MUST upload every shard + manifest via the S3-compatible PUT API; credentials read from `secr` (key names listed in plan.md). Cache-Control headers: `public, max-age=31536000, immutable` on shards + geometry; `public, max-age=300` on `manifest.json`. For the local store impl, system MUST copy files into `<published_dir>/<version_label>/<key>` with no equivalent of Cache-Control (irrelevant for filesystem reads).
- **FR-010**: System MUST insert one `artifact_version` row only after every put succeeds, with `version_label`, `manifest_sha256`, `manifest_url` matching the URL the store's `put` call returned for the manifest (`https://<cdn>/...` for R2, `file://...` for local), and `data_source_versions_jsonb` matching the manifest's `source_versions`.
- **FR-011**: System MUST wire the build + publish flows into `main.rs`: `ingestion build <output-dir> <version-label>` runs local-only build; `ingestion publish <version-label> [--destination=local|r2] [--local-dir=<path>] [--dry-run]` runs publish + record. Pre-product-ship the launchd job uses `--destination=local`; post-ship it flips to `--destination=r2` via a one-flag edit to the plist template.
- **FR-012**: System MUST cover the pure-function helpers (`collect_data_source_versions`, `apply_source_preference_merge`, manifest construction, content hashing) with TDD unit tests per Constitution VII. DB-touching helpers (`read_candidate_values`, `insert_artifact_version`) are covered by integration tests against `eafora_test`. Both store impls have integration tests: `LocalArtifactStore` against a `tempdir()`, `R2ArtifactStore` against a real R2 bucket once (manual verification per FR-014); `DryrunArtifactStore` is the test-time impl that exercises the publish orchestrator end-to-end without I/O.
...
- **FR-014**: System MUST verify the R2 store impl works end-to-end against a real R2 bucket at least once before PR B integrates (manual run; documented in task T034). After that one verification, the periodic launchd job uses the local destination until product ships.
