...
When publish runs, either the entire artifact set reaches the configured destination AND the `artifact_version` row is inserted, or nothing is recorded. There's no half-state where `artifact_version` claims a publication that the destination can't actually serve. The invariant generalizes from the architecture's Cloudflare R2-only framing: **`artifact_version` rows mean "fetchable from the destination recorded in `manifest_url`"** — `file://` for local destination, `https://...cdn...` for Cloudflare R2.

**Acceptance Scenarios**:

1. **Given** a Cloudflare R2 PUT fails partway through publish (network blip, 5xx from Cloudflare R2), **When** publish returns the error, **Then** `artifact_version` has no row for that version_label, AND the local build remains on disk so the developer can retry without re-running `build`. Any shards that succeeded upload before the failure remain in Cloudflare R2 — they're idempotently overwritten on retry (content-hashed key + same bytes = no-op PUT).
...
`ingestion delete <version-label> --destination=local|cloudflare-r2` removes a published artifact's files from its destination AND deletes the corresponding `artifact_version` row, atomically. Operationally most important for **local storage**: the pre-ship periodic job accumulates one build per week (~5-10MB each), so over years a Mac mini's published-directory grows; deleting old versions reclaims space. Less critical for Cloudflare R2 — the free tier covers years of accumulation per `project_eafora_r2_pricing.md` — but the same flow handles both destinations uniformly.

**Acceptance Scenarios**:

1. **Given** an `artifact_version` row for `2026-05-18` published to local destination at `${repo_dir}/published/2026-05-18/`, **When** `ingestion delete 2026-05-18 --destination=local --local-dir ${repo_dir}/published` runs, **Then** every file under `${repo_dir}/published/2026-05-18/` is removed (including the version directory itself), AND the `artifact_version` row for `2026-05-18` is deleted; subsequent `select * from artifact_version where version_label='2026-05-18'` returns zero rows.
2. **Given** an `artifact_version` row for `2026-05-18` published to Cloudflare R2, **When** `ingestion delete 2026-05-18 --destination=cloudflare-r2` runs, **Then** every shard + geometry + manifest under that version's content-hashed keys is DELETEd from the bucket via signed S3 DELETE; the `artifact_version` row is deleted; the CDN no longer serves the manifest URL (returns 404 after the 5-minute manifest cache TTL expires).
3. **Given** delete fails partway through (network blip mid-DELETE for Cloudflare R2; permission denied removing a file locally), **When** delete returns the error, **Then** the `artifact_version` row REMAINS (so the operator can see something needs cleanup), the destination is in a partially-deleted state; recovery is re-running `delete` with `--force` which skips any already-missing files.
...
- **FR-015**: System MUST implement `delete_artifact(store, pool, version_label) -> Result<(), AppError>` and a `ingestion delete <version-label> --destination=local|cloudflare-r2 [--force]` CLI dispatch. Semantics: read the `artifact_version` row, read the manifest from the destination, call `store.delete(key)` for every referenced file (shards + geometry + manifest), then DELETE the `artifact_version` row. If any file delete fails partway and `--force` is not set, the `artifact_version` row stays so the operator sees something needs cleanup; the operator re-runs with `--force` to skip already-missing files and complete the deletion.
- **FR-016**: The `ArtifactStore` trait MUST include `async fn delete(&self, key: &str) -> Result<(), AppError>` alongside `put`, implemented by `LocalArtifactStore` (filesystem remove + empty-dir cleanup), `CloudflareR2ArtifactStore` (signed S3 DELETE), and `DryrunArtifactStore` (record + no-op).
