# Feature Specification: Artifact builder + R2 publish
...
**Input**: User description: "Artifact builder + R2 publish — reads the canonical store, emits FlatGeobuf geometry + per-statistic / per-license-class SQLite shards + content-hashed manifest into a local build directory, uploads to Cloudflare R2 and records an artifact_version row. Implements the design in docs/architecture/ingestion.md §Artifact builder and §R2 upload."

## Scenarios & Testing *(mandatory)*

### Scheduled artifact publication (P1)

After the weekly WB WDI ingestion run completes on the Mac mini, the launchd-managed schedule chains a build + publish: every value with `superseded is null` in `statistic_value` is rolled into the source-preference merge, emitted into SQLite + FlatGeobuf shards with content-hashed filenames, written to manifest.json, uploaded to R2 under a stable `version_label`, and recorded in `artifact_version`. Clients fetching the next version see the updated data. Without this, the canonical store has fresh data but no client can reach it.

**Acceptance Scenarios**:

1. **Given** a canonical store with WB WDI TFR values for ≈200 countries × ≈65 years (≈13k current rows) and the wb_wdi data_source row, **When** `ingestion build ./build-output 2026-05-25` runs followed by `ingestion publish 2026-05-25`, **Then** `./build-output/` contains exactly one SQLite shard (`data/tfr-base-<sha8>.sqlite`), one FlatGeobuf geometry shard (`geometry/world-50m-<sha8>.fgb`), and one `manifest.json`; every shard is published under its content-hashed key in the configured R2 bucket; one `artifact_version` row is inserted with `version_label='2026-05-25'`, the manifest's SHA-256, and `data_source_versions_jsonb={"wb_wdi": "2025-Q1"}` (or whatever revision was current).
2. **Given** a successful publish completed for `2026-05-18`, **When** the canonical store changes (a WB WDI revision lands), **Then** the next `ingestion build` produces shards whose content hashes differ from the prior build (because at least one cell's value changed); the manifest references the new hashes; publish uploads the new files alongside the old ones (content-hashed naming makes uploads additive); a new `artifact_version` row records the new label.
3. **Given** R2 credentials are configured via `secr` and the build directory is populated, **When** `ingestion publish 2026-05-25` runs, **Then** each shard + manifest is uploaded via S3 PUT to the R2 bucket with `Cache-Control: public, max-age=31536000, immutable` on shards and `Cache-Control: public, max-age=300` on `manifest.json`; the inserted `artifact_version.manifest_url` resolves to the actual CDN URL after upload.

---

### Manual local build for inspection (P2)

A developer runs `ingestion build <output-dir> <version-label>` from a shell to produce artifact files locally without uploading. Used to inspect the shard contents, diff against a prior build, verify the manifest, or stage a build before authorizing the publish step. The local build does NOT write to `artifact_version` — that table records published artifacts only.
...
When publish runs, either the entire artifact set lands in R2 + the `artifact_version` row is inserted, or nothing is recorded. There's no half-state where `artifact_version` claims a publication that R2 doesn't actually serve. The architecture's invariant: `artifact_version` rows mean "fetchable from the CDN."

**Acceptance Scenarios**:

1. **Given** R2 PUT for one of the shards fails partway through publish (network blip, 5xx from R2), **When** publish returns the error, **Then** `artifact_version` has no row for that version_label, AND the local build remains on disk so the developer can retry without re-running `build`. Any shards that succeeded upload before the failure remain in R2 — they're idempotently overwritten on retry (content-hashed key + same bytes = no-op PUT).
2. **Given** every shard + manifest uploads successfully, **When** `insert_artifact_version` then fails (DB connection drops, unique-constraint violation on `version_label`), **Then** the operator gets a clear AppError naming what was uploaded but not recorded; manual recovery is re-running `publish` with the same version_label (which detects the existing CDN files via idempotent re-upload and either re-attempts the insert or surfaces the conflict).
...
- **R2 credentials missing or invalid** — publish fails fast with an `AppError` naming the secret it tried to read; no R2 PUTs attempted; no `artifact_version` row.
...
- **Statistic has no rows in canonical store** — the corresponding SQLite shard is NOT emitted (zero-byte SQLite would be misleading); the manifest's `statistics` map omits that statistic entirely.
...
- **FR-001**: System MUST implement `build_artifacts(pool, output_dir, version_label) -> Result<LocalArtifactBuild, AppError>` exactly per `docs/architecture/ingestion.md` §Artifact builder, including all seven named helpers (`read_candidate_values`, `collect_data_source_versions`, `apply_source_preference_merge`, `emit_sqlite_shards`, `emit_geometry_flatgeobuf`, `compute_content_hashes`, `emit_manifest`).
- **FR-002**: System MUST implement `upload_artifacts_to_r2(pool, build) -> Result<ArtifactVersion, AppError>` exactly per §R2 upload, including `upload_files_to_r2` and `insert_artifact_version`. The local build MUST NOT be modified or deleted by upload; recovery from upload failure is re-running publish.
...
- **FR-009**: System MUST upload every shard + the manifest to the configured R2 bucket via the S3-compatible PUT API. Credentials are read from `secr`-encrypted secrets (key names TBD in plan). Cache-Control headers: `public, max-age=31536000, immutable` on shards + geometry; `public, max-age=300` on `manifest.json`.
- **FR-010**: System MUST insert one `artifact_version` row only after every upload succeeds, with `version_label`, `manifest_sha256`, `manifest_url` resolving to the CDN URL the manifest is now fetchable from, and `data_source_versions_jsonb` matching the manifest's `source_versions`.
- **FR-011**: System MUST wire the build + publish flows into `main.rs`: `ingestion build <output-dir> <version-label>` runs local-only build; `ingestion publish <version-label>` runs upload + record. The `--upload` flag on `build` chains the publish step inline using the just-completed local build (avoiding a round trip through disk for the scheduled case).
- **FR-012**: System MUST cover the pure-function helpers (`collect_data_source_versions`, `apply_source_preference_merge`, manifest construction, content hashing) with TDD unit tests per Constitution VII. DB-touching helpers (`read_candidate_values`, `insert_artifact_version`) are covered by integration tests against `eafora_test`. The R2 upload path is covered by an integration test using a mock S3 server (or `--dry-run` flag — see Assumptions).
- **FR-013**: System MUST treat the local build directory as the source of truth between build and publish — `publish` reads the manifest + shards from disk and uploads what's there. The CLI MUST NOT re-execute `build_artifacts` inside publish; the operator runs `build` explicitly first, OR uses `build --upload` to chain them.

### Key Entities

- **`LocalArtifactBuild`**: a plain owned struct returned by `build_artifacts`, holding `version_label`, `hashed_outputs` (every shard + geometry's path + size + hash), `manifest_output` (the manifest's path + size + hash), and `data_source_versions` (the BTreeMap snapshot). Passed to `upload_artifacts_to_r2` to publish.
...
- **`MergedValue`**: a row produced by the source-preference merge, ready for SQLite shard emission. Carries `(region_id, statistic_id, period_start, period_end, license_class, value, data_status, data_source_revision)`.
...
- **SC-002**: A scheduled `build + publish` cycle completes end-to-end (read canonical → emit shards → download + process geometry → upload to R2 → insert artifact_version) in under 60 seconds on the Mac mini.
...
- **SC-005**: The pure-function helpers (`collect_data_source_versions`, `apply_source_preference_merge`, manifest serialization, SHA-256 hashing) achieve ≥90% line coverage in the test suite. DB and HTTP helpers are exercised by integration tests; their coverage is informational, not gating.
...
- Per Constitution Principle V (explicit over implicit), the R2 upload uses raw `reqwest` against the S3 API rather than the `aws-sdk-s3` crate. The architecture doc names this preference.
...
- **Principle V (Explicit over implicit)**: applies — raw `reqwest` for R2 (no SDK), hand-written SQL, manual file orchestration. No frameworks hiding the wire/disk operations.
