...
> - **Local repository config**: takes `root: PathBuf` + `public_base_url: String` (no `version_label` stored on the struct; the orchestrator prefixes the version label into each key). `--root` is optional and falls back to `EAFORA_LOCAL_REPOSITORY_ROOT` env var.
