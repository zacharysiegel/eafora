...
> **Status note (post-implementation review):** Significant portions of this plan were superseded during the build-and-review pass for PRs A/B. The current ground truth is the spec (`spec.md`) plus the source code; this plan is preserved for historical context. The most material divergences:
>
> - **`aws-sdk-s3` (1.135.*) + `aws-config` (1.8.*) replaced `aws-sigv4` (1.3.*).** The "explicit over implicit" preference still holds; the SDK was chosen after weighing the cost of hand-signing every PUT against the SDK's correctness on edge cases (multipart, retries, regional quirks).
> - **Storage dispatch is an enum (`ArtifactRepositoryKind`), not a generic-monomorphized trait.** The `ArtifactRepository` trait still exists (native `async fn`, no `async-trait` dep), but the orchestrator takes `&ArtifactRepositoryKind` and match-dispatches to the variant's concrete impl rather than monomorphizing per `S: ArtifactRepository`.
> - **`Dryrun` → `Dry`** (CLI subcommand `publish dry`, struct `DryArtifactRepository`).
> - **`LocalArtifactBuild` → `BuildReport`; `data_source_versions` → `data_source_revisions`; `MergedValue` → `ResolvedValue`.**
> - **CLI shape**: nested sub-subcommands rather than `--destination=` flags. `ingestion ingest source|all`, `ingestion build`, `ingestion publish local|cloudflare-r2|dry [<artifact-dir>] [--build]`. `--build` is a global on `publish` that chains build + publish.
> - **Auto-generated version labels.** `version_label` is no longer a CLI argument; it's generated by `crate::version_label::generate` as `YYYY-MM-DD+<surname>` from a Nobel-laureate list, with a DB pre-check to guarantee uniqueness against `artifact_version`.
> - **Non-clobbering precheck instead of `ON CONFLICT (version_label) DO NOTHING`.** `read_artifact_version_exists` runs before any uploads and aborts with a clear error if the label is taken.
> - **Local repository config**: takes `root: PathBuf` + `public_base_url: String` (no `version_label` baked into the struct; the orchestrator prefixes the version label into each key). `--root` is optional and falls back to `EAFORA_LOCAL_REPOSITORY_ROOT` env var.
> - **Trait shape**: two methods — `async fn put_file(key, source_path, content_type)` and `fn url_for(key) -> String` — replacing the planned single `async fn put(key, body, cache_control)`. Cache-Control is set per-file inside the R2 impl, not as a trait parameter; `delete` is deferred to a future PR.
> - **`build` no longer takes positional args.** Output goes to `$EAFORA_ARTIFACTS_DIR/<auto-generated-label>/`. `publish <kind> <artifact-dir>` requires the bundle path positional unless `--build` is set.
> - **Logging convention** (added during review): `<message>; [key=value ...]`; see `docs/conventions/logging.md`.
> - **Type/symbol layout convention**: imports → consts → all type definitions → all functions; see `docs/conventions/types.md` and the in-repo style rules.
> - **`crate::filesystem`** module exists for path utilities; `crate::secrets` reads `secrets.yaml` decrypted via `secr` against `MASTER_SECRET` in `.env`; `crate::version_label` houses the Nobel-laureate generator and `nobel_surnames.txt`.
>
> The "Implementation phases" and "Phasing for PRs" sections below describe the originally planned breakdown; what actually shipped is visible in `git log impl-publish-flow`.
