...
PR B introduces the destination-agnostic `ArtifactStore` trait and three impls: local disk, Cloudflare R2, and a dry-run for tests. Pre-product-ship the periodic job runs with `--destination=local`; the Cloudflare R2 store is verified once against a real bucket then sits behind the flag until launch.
...
- [ ] T025 [Tests-first] Write unit tests for the aws-sigv4 signing logic in `ingestion/src/artifact/store/cloudflare_cloudflare_r2.rs::#[cfg(test)] mod tests`: assert that a known input (PUT method, bucket/key, fixed timestamp, fixed credentials) produces the expected `Authorization` header per the AWS sigv4 spec.
- [ ] T026 Implement `CloudflareCloudflareR2ArtifactStore`. `put` signs a PUT request via `aws_sigv4::http_request::sign`, sets `Cache-Control` from the caller, issues via `reqwest`, returns the `<cdn_base_url>/<key>` URL.
...
- [ ] T035 Implement `delete_artifact(store: &dyn ArtifactStore, pool: &PgPool, version_label: &str) -> Result<(), AppError>` in `ingestion/src/artifact/publish.rs` (or a sibling `delete.rs`). Atomic semantics: read the `artifact_version` row → read the manifest from the destination → for each referenced file, call `store.delete(key)` → finally DELETE the `artifact_version` row. If any file delete fails partway, the operation aborts with an AppError and the `artifact_version` row stays (so the operator can see something needs cleanup) — but the destination is in a partially-deleted state. Recovery: re-run delete with `--force` to skip any already-missing files.
- [ ] T036 Extend the `ArtifactStore` trait with `async fn delete(&self, key: &str) -> Result<(), AppError>`. Implement on `LocalArtifactStore` (file remove + cleanup empty version dir), `CloudflareCloudflareR2ArtifactStore` (signed DELETE request), and `DryrunArtifactStore` (record + no-op).
- [ ] T037 [US] Wire `dispatch_delete` in `main.rs`: `ingestion delete <version-label> [--destination=local|cloudflare-r2] [--local-dir=<path>] [--force]`. Forces the operator to specify the destination (no default) since deletion is irreversible.
- [ ] T038 Integration test in `tests/artifact_integration.rs`: publish to a `LocalArtifactStore` in `tempdir()`, then delete, then assert: the published-dir for the version is gone, the `artifact_version` row is gone, the local build dir (the build-output, separate from the published-dir) is untouched.
...
- [ ] T042 [P] If implementation surfaced any divergence from `docs/architecture/ingestion.md` §Artifact builder or §Cloudflare R2 upload, propose an architecture amendment in a follow-up. Specifically expected: the architecture currently talks about `upload_artifacts_to_cloudflare_r2`; the as-built `publish_artifacts(store: &dyn ArtifactStore, ...)` lives in the same doc with the trait abstraction added, plus the new `delete_artifact` flow.
