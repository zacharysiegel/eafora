...
- [ ] T035 Implement `delete_artifact(store: &dyn ArtifactStore, pool: &PgPool, version_label: &str) -> Result<DeleteReport, AppError>` in `ingestion/src/artifact/delete.rs`. Well-ordered, NOT cross-system atomic: open a Postgres transaction, SELECT the row, read the manifest from the destination to enumerate referenced file keys, DELETE the row, COMMIT — then best-effort `store.delete(key)` per file. Per-file failures accumulate into `DeleteReport.orphan_files` + log a warning per file; the operation returns Ok with the report. The row deletion's atomicity is real (Postgres transaction); the file deletes are not — orphans are a known + reported state, not a failure.
