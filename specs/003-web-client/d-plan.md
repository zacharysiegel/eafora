...
- Phase E: wrangler, production `_headers`, perf-budget, build-time prerender. Precompression was dropped; a probe deploy showed Workers Assets does not serve uploaded `.br` siblings (spec FR-005).
...
- D3 first shipped the two startup fetches as sequential awaits, which the D2 Step 3 note had flagged as the fallback to avoid. They now run under `tokio::join!`, enabled by adding `tokio/macros` to the `hydrate` feature; `tokio` was already a dependency and `tokio-macros` was already in `Cargo.lock` via `ssr`, so no crate was added.
