...
`build-site.sh` runs `cargo leptos build --release`, then `web prerender`, then rewrites the discovery document, in that order, because a build empties the site root and what follows has to survive it. The rewrite matters: the committed `web/static/discovery` names `/repository`, which is where `ingestion publish local` writes and what `cargo leptos watch` serves, and a deploy has to name `https://repository.eafora.org` instead. The same value reaches the wasm through `EAFORA_REPOSITORY_BASE_URL`, because the speculative manifest fetch that races discovery uses the compiled-in base rather than the fetched document. `verify-site-tree.sh` refuses to deploy a tree whose discovery document still names a relative base. It also sets `LEPTOS_HASH_FILES=true`, which gives the assets content-hashed filenames so the deploy can serve them as immutable. That setting lives here rather than in `web/Cargo.toml` on purpose: cargo-leptos only re-hashes on a full build, so under `cargo leptos watch` an incremental rebuild would write unhashed files while the page still asked for the previous build's hashed names, and the browser would silently load stale wasm.

`build-site.sh` refuses to run while `cargo leptos watch` is listening, because both write `target/site`: a release build replaces the tree the dev server is serving, and the rebuild that follows restores the dev files under their stable names, which leaves a browser holding a shim from one build and a module from another. Pass `--force` if you mean to, and hard-reload afterwards.
