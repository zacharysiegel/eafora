...
**Home view.** The home viewport fills the surface vertically with the **drawn geometry's latitude span** (`GeometryLayer::bounding_box`, roughly 56°S to 84°N — the layer carries no Antarctica), **horizontally centered on Washington DC's longitude** (≈ 77.04° W; the implementation defines a `HOME_CENTER` constant at ≈ 38.91° N, 77.04° W). Vertically it is centered on the content's midpoint (≈ 14°N, since land skews north), not the equator, so no empty ocean band shows below the southernmost land. Longitude runs at the same isotropic scale, so the surface width shows as much as fits; the remaining longitude is reached by horizontal pan (C3.3), with the wraparound placing DC at the middle. A surface wider than the content's aspect would repeat the world horizontally; that case is not special-cased for C3.1.

**Resize.** On resize the surface aspect changes, so the driver recomputes the viewport with `fill_height` around the same center and content extent. The driver calls it at startup and in its resize handler.

**Where it lives.** The `fill_height` function is a pure `shared::map` helper (no `web_sys`, host-testable). The driver computes the geometry's bounding box once at startup and owns the home-view construction (calls `fill_height` with that box's latitude extent + `HOME_CENTER`) at attach and on resize.

> **Deviation (post-review).** The design originally fit the whole world into the surface aspect by *containing* it (expanding the surplus axis). Visual review showed that produced horizontal tiling on a surface wider than the map and vertical paper margins on a narrower one, so it was replaced with fill-vertically (`Viewport::fill_height`) and the `Viewport::fit` contain helper was removed (a contain-style variant returns with C3.4). A second visual pass showed the ±85° framing left a large empty band at the bottom — the southern ocean below ~56°S, with no Antarctica in the layer — so the home view now frames the geometry's own bounding box (`GeometryLayer::bounding_box`) rather than a `WORLD_BOUNDS` constant, which was removed.
...
**web** — The map viewport now fills the canvas vertically with the drawn geometry's latitude span (framing the content, not the ±85° world, so no empty ocean band shows below the southernmost land) and is recomputed on resize; the home view centers horizontally on Washington DC's longitude, with longitude beyond the canvas width reached by panning.
