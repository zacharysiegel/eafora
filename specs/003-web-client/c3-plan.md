...
The camera start is driven off the raw `RegionHit`, not off the selection's change result. Selection short-circuits (publishes nothing) when the tapped region equals the selected one, so hanging the camera off that result would make re-tapping an already-selected country (after panning away) a no-op. Instead, on `pointerup` for an unmoved tap, the driver resolves the `RegionHit` once (`region_at`), starts the camera off that hit's `framing`, and then hands the same hit to `select_region` for the panel publish (which keeps its own unchanged short-circuit). On a country hit the camera computes the target and, if the current viewport differs from it beyond a small tolerance, animates (a re-tap while already framed is a no-op; a re-tap after panning away re-frames). A tap on open ocean deselects and starts no animation. Selection and animation are independent effects of the tap: the selection state paints on the next frame; the animation moves the viewport over its duration. The camera start stays in the driver (it needs the rAF timestamp), inside the `DRIVER.with_borrow_mut` block `handle_pointer_up` already holds.

> Deviation from the original plan: `select_region_at(surface_point)` (which resolved the hit internally) was refactored to `select_region(Option<RegionHit>)` so the tap handler resolves the `RegionHit` exactly once and shares it between the camera start and the selection publish, rather than hit-testing twice.
