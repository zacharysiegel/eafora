...
- `web/static/embedded_artifacts/` (gitignored) — a downsampled bundle: `manifest.json` (schema v1) + `geometry/world-50m-<sha8>.fgb` (full 1:50m) + `data/<statistic>-base-<sha8>.sqlite` (most-recent-year-per-country). Produced by `ingestion build --downsampled` (Phase 0b) or, interim, hand-built.
