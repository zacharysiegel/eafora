...
- `shared::license::DistributionContext` (`license/license.rs`) — `FirstParty` when serving eafora.org itself, `ThirdParty` when running inside another party's site (Base shard class only). Resolved once per session from the deployment, never from which bundle was loaded.
...
1. **Embedded first paint**: fetch the embedded manifest from `static/embedded_artifacts/`, `cache.put` its files, `Bundle::open(cache, embedded_version, distribution_context)`, publish on `BUNDLE_TX`. This is the same fetch→cache→open path as the live bundle, pointed at the same origin.
