...
- **FR-010**: Every statistic MUST have a `StatisticKind` variant, and ingestion MUST resolve statistics through it rather than through a bare code string. The enum is the authoritative list of statistics the codebase knows; a source module naming its statistic by string literal escapes that.
- **FR-010a**: A statistic code that a build cannot parse MUST be skipped with a warning rather than failing the build, and the same MUST hold for an unrecognized key when reading a published manifest. The database can be migrated ahead of the binary reading it, and a client reads whichever manifest the repository currently serves while being cached from an earlier release; in both directions the reader can legitimately meet a code it has no variant for. Failing would take down everything that does work, so the recognized entries are kept and the rest are dropped with a warning.
- **FR-010b**: Phase A and Phase B MUST reach the trunk together. Phase A alone would publish a bundle carrying completed cohort fertility while the client still draws it on a calendar axis as a single instant, which is the misstatement Phase B exists to prevent. They stay separate changes for review and land at the same time, so no schema column is needed to withhold a statistic from clients.
