...
- **FR-010a**: A statistic code that a build cannot parse MUST be skipped with a warning rather than failing the artifact build, because the database can be migrated ahead of the binary reading it. Manifest parsing stays strict: a bundle naming a statistic the reader has no variant for is a release-ordering fault, so the client MUST be deployed before a bundle carrying a new statistic is published.
