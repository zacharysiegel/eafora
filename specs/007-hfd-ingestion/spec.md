...
- An HFD code names a territory within a country rather than the country (`DEUTE`, `DEUTW`, `GBRTENW`, `GBR_SCO`, `GBR_NIR`): no canonical region matches, and the run records a warning and continues.
- A value is absent, which HFD writes as `.`: the cell is skipped rather than parsed as zero.
- Both HFD and World Bank WDI supply a cell for the same region, statistic, and period: the higher-priority source wins that cell. A period only the lower-priority source covers still emits its value, so a preferred source's narrower coverage does not truncate the series.
...
- **FR-006**: System MUST map each row's `Code` to a canonical region: 29 codes are bare ISO 3166-1 alpha-3 and map directly, and `DEUTNP`, `FRATNP`, `GBR_NP` are national totals that MUST map to Germany, France, and the United Kingdom. `DEUTE`, `DEUTW`, `GBRTENW`, `GBR_NIR`, `GBR_SCO` are territories within a country, have no canonical region, and MUST produce a warning rather than a row. A bare code that matches no canonical region MUST also warn rather than stop the run.
...
- **FR-008**: System MUST register HFD in `data_source` via a seed migration with `code='hfd'`, `license_class='attribution'`, `license_name='CC BY 4.0'`, and a `preference_rank` ahead of World Bank WDI, because HFD recomputes from national registers on one method. Selection is per `(region, statistic, period)` cell, so no per-statistic or per-region selection row is seeded.
...
- **FR-010**: System MUST add a nullable `released timestamp with time zone` column to `statistic`, and the artifact build MUST consider only released statistics. This is a deployment control, distinct from whether a reader recognizes the code at all: a statistic can be fully known to the codebase and still deliberately withheld. The column follows the schema's existing idiom for state changes, nullable timestamps rather than booleans as with `statistic_value.superseded`, and carries when the statistic began being offered for the same storage. It is named `released` rather than `published` because `data_source_publication.published` already means the upstream's own publication date.
- **FR-010a**: Every statistic MUST have a `StatisticKind` variant, and ingestion MUST resolve statistics through it rather than through a bare code string. The enum is the authoritative list of statistics the codebase knows; a source module naming its statistic by string literal escapes that. Phase A therefore lands the variant, and the release column rather than the enum is what withholds a statistic from clients.
- **FR-010b**: A statistic code that a build cannot parse MUST be skipped with a warning rather than failing the build, and the same MUST hold for an unrecognized key when reading a published manifest. The database can be migrated ahead of the binary reading it, and a client reads whichever manifest the repository currently serves while being cached from an earlier release; in both directions the reader can legitimately meet a code it has no variant for. Failing would take down everything that does work, so the recognized entries are kept and the rest are dropped with a warning.
...
- **FR-012a**: An absent `CCF` MUST NOT warn. It means the cohort has not completed childbearing, which is the normal state of the newest ten cohorts of every country, 364 of 1701 rows in the 2026-07-02 release. Warning per row would emit hundreds of warnings a run and bury the ones that matter.
- **FR-013**: System MUST provide checked-in sample files under `ingestion/samples/hfd/` covering a happy path, an absent value, an unmapped territory code, and a country whose every value is absent. Samples MUST be replayable without network access.
...
- The login posts `Email`, `Password`, `ReturnUrl`, and `__RequestVerificationToken` to `/Account/Login`, the token being paired with a cookie the form's own response sets. `reqwest`'s `cookies` feature carries both through the login redirect.
- Cohorts are single years, confirmed against the 2026-07-02 release: 1876 to 1985 across the file, with per-country ceilings between 1963 and 1975. FR-018's span therefore renders one year wide for this statistic; it exists for the multi-year cohorts a later source may bring.
- Coverage is uneven by design. Each country's series ends where its cohorts stop being complete, so the map will show no-data regions at the newest cohorts even for countries HFD covers well. Phase B's axis work is what keeps that legible.
- Countries in scope are the 32 that FR-006 maps, from HFD's 37 codes.
...
- **HFD's subnational territories.** East and West Germany and the UK's constituent countries have no canonical region. Adding them is a region-hierarchy decision, not an ingestion one.
