...
Three migrations across the three phases, dbmate-timestamped when written. `ingestion/db/schema.sql` is regenerated by dbmate afterward and committed with each.
...
Phase C's period TFR needs no migration of its own; it writes to the `tfr` statistic, which is already seeded and released.
...
3. **Normalize against the existing `tfr` statistic,** which is already seeded and released.
