...
Three phases, one PR each, on a linear stack.

- **A, ingestion.** The adapter, the seed, and real rows in `statistic_value`. No client changes beyond what adding the enum variant forces.
- **B, presentation.** The client learns to render a cohort, and the final migration releases the statistic.
- **C, period total fertility.** HFD's period TFR, from a file already inside the archive A downloads, plus the per-cell source priority it needs to be worth having.
...
Five places the spec and the code disagree. Code is canonical except where it has drifted from the schema, which is the first item.

- **The code drifted from the schema on source selection, and Phase C returns it.** `data_source.preference_rank` is in the initial schema, commented "drives data-source-preference merge; lower wins; ties broken deterministically by `data_source.id`". The `source_choice` table arrived three months later and displaced it, narrowing selection to one source per `(region, statistic, license_shard_class)` series and leaving `preference_rank` read by nothing. The spec describes the schema's design, not the code's. Phase C restores it: each `(region, statistic, period)` cell takes the highest-priority source holding a value for it, so a preferred source's coverage gaps are filled by the next source rather than becoming gaps in the published series.
...
One migration, in Phase A. `ingestion/db/schema.sql` is regenerated by dbmate afterward and committed with each.

### Phase A: `seed_hfd_and_completed_cohort_fertility`
...
- `statistic`: `code='ccf'`, `name_en='Completed cohort fertility'`, `name_abbreviated_en='CCF'`, units in the same idiom as `tfr`'s "children per woman".
...
Phase B and Phase C need no migration; Phase C writes to the `tfr` statistic, which the initial seed already carries.
...
2. **Region mapping.** A const table aliasing each of HFD's three suffixed whole-population codes to its ISO 3166-1 alpha-3 (`DEUTNP`→`DEU`, `FRATNP`→`FRA`, `GBR_NP`→`GBR`), as one struct rather than two parallel arrays. Every other code resolves through `canonical_db::find_country_by_iso3` unchanged, and one that matches nothing warns.
...
## Phase C steps, period total fertility and per-cell source priority

Two changes in one PR: HFD's period TFR, and the source priority that makes it worth having. Separately, each is pointless. Priority resolution changes no published output while total fertility has one source, and HFD's period TFR without it would shorten 26 countries' series rather than lengthen 13.

The resolution work rewrites `ingestion/src/artifact/source_choice.rs` as `source_priority.rs`. It is mostly deletion.

### Per-cell source priority
...
### HFD period total fertility

HFD's `tfrRR.txt` is already inside the archive Phase A downloads, so this adds a parse and no new transport. With priority resolution in place, HFD's 13 countries reaching 2024 or 2025 gain recency while the 26 whose series end earlier keep their World Bank values for the later periods.
...
3. **Normalize against the existing `tfr` statistic,** which the initial seed already carries.
...
- Parser and mapping: host unit tests in `hfd_model.rs` and `hfd_adapter.rs` against `ingestion/samples/hfd/tfrVH.txt`, which covers a plain code, an aliased code that maps, two codes that must warn, an absent `CCF`, and a country with no values at all (FR-013).
...
The adapter authenticates against HFD, downloads the by-statistic archive once per run, parses the cohort file by column name, and maps HFD's 37 country codes onto 32 canonical regions; the five that match no canonical region produce warnings. A statistic code the build cannot parse is now skipped with a warning rather than failing the build, as is an unrecognized key in a published manifest, so a reader older than what it is reading keeps what it does understand.

Must reach the trunk with the presentation change that draws a cohort as a range.
...
Adds HFD's period total fertility, and resolves each published cell to the highest-priority source that holds a value for it.

Thirteen countries gain a more recent total fertility figure than the World Bank publishes, one of them 2025. Selection previously picked one source per region and statistic from a configuration table, which would have made HFD's narrower coverage into gaps; resolving per `(region, statistic, period)` cell instead lets the countries whose HFD series ends earlier keep their World Bank values for the later periods. Restores `data_source.preference_rank` to the role the schema documents for it and drops the `source_choice` table.
