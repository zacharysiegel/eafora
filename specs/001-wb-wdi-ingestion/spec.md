...
- **FR-007**: System MUST return an `IngestReport` containing `values_inserted` (new cells, no prior row), `values_revised` (cells where supersede + insert pair fired), and `values_unchanged` (cells where the existing current row already matched) counts plus `upstream_revision` (the publication's revision label), `started` / `finished` timestamps, and any `IngestWarning` instances accumulated during the run.
...
- **SC-003**: A re-run with no upstream change produces zero DB writes; the IngestReport shows `values_inserted = 0`, `values_revised = 0`, `values_unchanged = N` where N is the row count for WB WDI in `statistic_value` (filtered to `superseded is null`).
