...
│   ├── artifact/
│   │   ├── mod.rs                          # pub mod manifest; pub mod bundle; pub mod cache; pub mod discovery; pub mod geometry; pub use {manifest,bundle,cache,discovery,geometry}::*;
...
│   │   └── geometry.rs                     # GeometryLayer, parse_geometry_layer, CountryFeature, Polygon, BoundingBox, plus producer/consumer-shared constants: GEOMETRY_LAYER_NAME, GEOMETRY_FILENAME_STEM, FEATURE_COLUMN_ISO3, FEATURE_COLUMN_NAME_EN, SHARD_FILENAME_EXTENSION, GEOMETRY_FILENAME_EXTENSION (per FR-020f)
│   ├── license/
...
│       ├── vfs.rs                          # DEFERRED to 006 (sqlite-wasm-rs has no Connection type; bridge API defined by the renderer). Not created in 005; sqlite/mod.rs declares only `pub mod schema;`.
...
| D  | `impl-005-license-bundle-vfs`           | 7, 8 (US6 + US5)                         | `impl-005-manifest-discovery-cache` | `shared::license::DistributionContext` + `authorized_classes`. `shared::artifact::geometry::GeometryLayer` + geometry constants (FR-020f) + producer-side `ingestion/src/artifact/writer/flatgeobuf.rs` rewrite. `shared::sqlite::schema` (FR-020b through FR-020e: constants + DDL + header-validate) + producer-side `ingestion/src/artifact/writer/sqlite.rs` rewrite. `shared::artifact::bundle::Bundle` + `Bundle::open(version_label, &cache, ctx)` (generic over `C: ArtifactCache`, not `&dyn` — AFIT is not dyn-compatible). The hot-swap channel is `tokio::sync::watch`, used by consumers directly (the thin `bundle_watch` re-export was dropped; shared's tokio dep is dev-only). **The SQLite VFS / `Connection` bridge (FR-020) is DEFERRED to 006** (`sqlite-wasm-rs` has no `Connection` type; API defined by the renderer). The renderer (006) is unblocked on data shapes at the end of this PR; it builds its own connection bridge. ~600 LOC + tests. |
...
Each branch starts with the `>>> branch: <name>` marker commit per the constitution + `feedback_branch_marker_commits` (use `./scripts/git/branch-init.sh <name>`). Each PR's body matches `feedback_pr_description_style`: opens with a verb describing the change; tight summary prose; no chat narration; no file enumeration; no "Next phase" pointers.

Per `feedback_squash_merge_and_rebase_onto`: PRs integrate via `scripts/git/pr-integrate.sh`; for any stacked child whose parent was squash-merged, use `git rebase --onto master <former-parent>` to rebase the child onto master.

## Brief PR description (per `feedback_pr_description_style`)

> Adds a new `shared/` Cargo workspace member that compiles for both host (Apple Silicon) and `wasm32-unknown-unknown`. Extracts the canonical-store enums + manifest schema + SHA-256 helpers from `ingestion/`; ingestion imports them from `shared::` directly (no forwarding re-exports), and ingestion's `canonical_model.rs` is renamed to `canonical_entity.rs` once the canonical Models leave. Introduces a new `manifest_schema_version: u32` field on the manifest (first key; v1 = 1) as a forward-compat gate for v2+ shape changes. Defines the cross-platform `ArtifactCache` async trait (stable AFIT, no `async-trait` crate). Defines `DiscoveryDocument` + `parse_discovery_document`. Implements `Bundle::open(version_label, &cache, distribution_context)` returning a `Send + Sync` `Bundle` of pure parsed data (manifest + eagerly-parsed GeometryLayer + license-filtered shard bytes; no SQLite Connection — the consuming renderer in 006 opens its own thread-local). Adds `shared::sqlite::schema` as the shared producer / consumer SQLite-shard contract (constants for the `application_id` / `user_version` magic numbers + every table / column / index name + the `PERIOD_DATE_FORMAT`; `shard_schema_ddl()` builds the schema DDL from those constants; `validate_shard_header()` is the consumer-side "is this an Eafora shard with a version I understand?" gate). Producer-side `ingestion/src/artifact/writer/sqlite.rs` now uses those constants instead of its own copies; `ingestion/src/artifact/publish.rs::load_build_report_from_disk` now uses `shared::artifact::manifest::parse_manifest` instead of its private deserializer. Re-exports `tokio::sync::watch` for the bundle hot-swap channel. Ships the SQLite `Vec<u8>`-backed VFS for the wasm32 target. Architecture docs (`client.md` §Manifest schema, `ingestion.md` §Manifest format, `overview.md` §Artifact format) updated to include `manifest_schema_version`. Producer-side tests continue to pass; new test suite covers `parse_manifest`, `parse_discovery_document`, `verify_sha256`, `DistributionContext::authorized_classes`, `Bundle::open`, `validate_shard_header`, and `shard_schema_ddl` on both targets.

## Status note (post-implementation review)

Implemented across PRs A–E (`impl-005-foundational` → `impl-005-canonical-types` → `impl-005-manifest-discovery-cache` → `impl-005-license-bundle-vfs` → `impl-005-wasm-tests-polish`). Deviations from the plan above:

- **SQLite VFS / `Connection` bridge deferred to 006.** `sqlite-wasm-rs` exposes only raw C bindings (no `Connection` type), and the bridge's API is shaped by the renderer's queries, which live in 006. 005 ships only `shared::sqlite::schema` (the shard contract); the `Vec<u8>`-backed VFS and `open_connection_from_bytes` are 006's. Consequently the wasm-only deps added for the VFS (`sqlite-wasm-rs`, `wasm-bindgen`, `wasm-bindgen-futures`, `js-sys`) were removed in PR E as dead; only `uuid`'s `js` feature remains on the wasm target (getrandom backend for the workspace `v7` feature).
- **No `tokio::sync::watch` re-export.** The planned `shared::artifact::bundle_watch` module was dropped — re-exporting a third-party type adds nothing. Consumers (003 / 004 / 006) use `tokio::sync::watch` directly; `shared`'s tokio dependency is dev-only (the test mock's `Mutex`).
- **`Bundle::open` is generic, not `&dyn`.** `open<C: ArtifactCache>(cache: &C, …)` — stable AFIT makes `ArtifactCache` non-dyn-compatible, and static dispatch is preferred regardless.
- **Geometry types renamed from the plan's working names.** `Feature` → `CountryFeature` (the layer is admin-0; every feature is a country), `FlatGeobufReader` → `GeometryLayer` (it owns bytes and reopens a transient reader per query; "reader" overstated it), `open_flatgeobuf_reader` → `parse_geometry_layer` (aligns with `parse_manifest` / `parse_discovery_document`). `Polygon` keeps the OGC name with `exterior` / `interiors` fields.
- **Test-only enum variants are unconditional, not feature-gated.** `StatisticKind::TestAlpha` etc. are plain variants; only their `TryFrom<&str>` parse arms are `#[cfg(test)]`-gated, so production rejects the test codes. The originally-used `testing` Cargo feature was removed, so `cargo test --workspace` needs no flag.
- **`cargo fmt` intentionally not run (T048).** The codebase is hand-formatted; rustfmt would reformat ~148 files against the owner's manual-formatting conventions. Formatting stays manual.
- **`module_inception` + `explicit_auto_deref` allowed workspace-wide.** Both fight deliberate conventions (the `mod.rs` layout; the `&mut *tx` threading idiom). `cargo clippy --workspace --all-targets -- -D warnings` is otherwise clean.
- **wasm32 test coverage is one runtime test, by the significant-divergence bar.** Only the geometry reader (`parse_geometry_layer`) runs under `#[wasm_bindgen_test]` (headless Chrome via `scripts/test/test-wasm.sh`), because flatgeobuf/geozero is the one dependency with a wasm-hostile runtime path (its writer traps on `env::temp_dir`); everything else is target-agnostic and covered by `cargo check --target wasm32`. The fixture is a committed 496-byte `shared/tests/samples/one-feature.fgb` loaded via `include_bytes!` on both targets.
