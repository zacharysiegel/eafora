...
- **wasm32 test coverage runs headless in Chrome.** Single-body tests gated via `#[cfg_attr(target_arch = "wasm32", wasm_bindgen_test)]` rather than duplicated. The `FgbWriter` fixture can't run on wasm32 (it opens a tempfile), so a 496-byte `shared/tests/samples/one-feature.fgb` is committed and loaded on the wasm path; the host path still generates it via the writer.
