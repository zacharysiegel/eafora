# Contract: shared/ public API

> Phase 1 output of `/speckit-plan` for 005-core-data. The complete list of public symbols `shared/` exposes to its consumers (`ingestion/` as the producer; `web/`, `ios/`, `android/` via 003 / 004 / future as the consumers; `shared/`'s own tests). Sourced from `data-model.md`.

## Contract type

Library crate: the contract is the public Rust API surface (types, traits, functions, constants). No HTTP routes, no CLI subcommands, no IPC schemas — `shared/` is purely consumed in-process by other Rust crates.

## Public symbol inventory

| Symbol                                                            | Module                          | Kind     | Stability |
|-------------------------------------------------------------------|---------------------------------|----------|-----------|
| `AppError`                                                        | `shared::error`                   | type     | locked    |
| `render_error_chain(error: &dyn Error) -> String`                | `shared::error`                   | function | locked    |
| `REVISION`                                                        | `shared` (crate root)             | const    | new       |
...
| `LicenseClass`                                                    | `shared::canonical::canonical_model` | type  | locked    |
| `LicenseShardClass`                                               | `shared::canonical::canonical_model` | type  | locked    |
| `SourceRevision`                                                  | `shared::canonical::canonical_model` | type  | locked    |
| `NaiveDatePeriod`                                                 | `shared::canonical::canonical_model` | type  | locked    |
| `Region` (Model only; Entity stays in ingestion)                  | `shared::canonical::canonical_model` | type  | locked    |
| `Country` (Model only; Entity stays in ingestion)                 | `shared::canonical::canonical_model` | type  | locked    |
| `Statistic` (Model only; Entity stays in ingestion)               | `shared::canonical::canonical_model` | type  | locked    |
| `DataSource` (Model only; Entity stays in ingestion)              | `shared::canonical::canonical_model` | type  | locked    |
...
| `ManifestEntry`                                                   | `shared::artifact::manifest`      | type     | new       |
| `parse_manifest(bytes: &[u8]) -> Result<Manifest, AppError>`      | `shared::artifact::manifest`      | function | new       |
| `DISCOVERY_SCHEMA_VERSION`                                        | `shared::artifact::discovery`     | const    | new       |
| `DISCOVERY_URL`                                                   | `shared::artifact::discovery`     | const    | new       |
| `DiscoveryDocument`                                               | `shared::artifact::discovery`     | type     | new       |
...
| `TABLE_STATISTIC_VALUE`, `TABLE_SHARD_KEY`                        | `shared::sqlite::schema`          | const    | new       |
| `INDEX_STATISTIC_VALUE_BY_REGION`                                 | `shared::sqlite::schema`          | const    | new       |
| `COL_REGION_ISO3`, `COL_REGION_ID`, `COL_PERIOD_START`, `COL_PERIOD_END`, `COL_VALUE`, `COL_DATA_STATUS`, `COL_DATA_SOURCE_CODE`, `COL_DATA_SOURCE_REVISION` | `shared::sqlite::schema` | const | new |
| `COL_STATISTIC_KIND`, `COL_LICENSE_SHARD_CLASS`                   | `shared::sqlite::schema`          | const    | new       |
| `PERIOD_DATE_FORMAT`                                              | `shared::sqlite::schema`          | const    | new       |
| `shard_schema_ddl() -> &'static str`                              | `shared::sqlite::schema`          | function | new       |
...
- **Re-export discipline**: every `ingestion/` import path that worked before this feature continues to work after (e.g. `use crate::canonical::canonical_model::StatisticKind` still resolves; the type is now `shared::canonical::canonical_model::StatisticKind` reached via a `pub use` in ingestion's `canonical_model.rs`). Per spec P1 acceptance #4.
