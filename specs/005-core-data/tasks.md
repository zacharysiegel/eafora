...
- [x] T015 [US1] Create `/Users/singularity/eafora/shared/src/canonical/canonical_model.rs` by moving from `/Users/singularity/eafora/ingestion/src/canonical/canonical_model.rs`:
  ...
  - **Deviation**: the test-only enum variants (`StatisticKind::TestAlpha`, `DataSourceKind::TestAlpha` / `TestBeta`) are gated `#[cfg(any(test, feature = "testing"))]`, not bare `#[cfg(test)]` — a `cfg(test)` gate does not cross the crate boundary, so dependent crates' tests couldn't see the variants. shared exposes a `testing` Cargo feature (`[features] testing = []`) for this.
...
- [x] T019 [US1] [P] Run `cargo test -p ingestion --features testing` to confirm the type extraction didn't regress any existing tests per spec SC-003. **The `--features testing` flag is REQUIRED**: ingestion's `testing` feature forwards `shared/testing` so the test-only enum variants exist; a dev-dependency feature would leak into the normal binary build via feature unification and make `main.rs`'s source-kind match non-exhaustive. The binary's match carries a `#[cfg(feature = "testing")]` catch-all arm for the test-only variants. Also fixed `ingestion/tests/` integration tests that still referenced the PR-A-deleted `ingestion::filesystem` (PR A only ran `--lib`, so the `tests/` targets never recompiled). Result: 43 tests green (29 lib + 14 integration, 1 ignored).
