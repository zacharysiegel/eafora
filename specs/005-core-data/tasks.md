...
- [x] T045 [P] **Deviation**: tests are NOT duplicated. Each target test keeps a single body with `#[test]` plus `#[cfg_attr(target_arch = "wasm32", wasm_bindgen_test::wasm_bindgen_test)]` (the async `bundle_open` uses `#[cfg_attr(not(target_arch = "wasm32"), tokio::test)]` + the wasm attribute, since tokio has no wasm runtime). Covered: `parse_manifest_round_trips_fixture_set`, `parse_discovery_document_round_trips_fixture`, the three `verify_sha256_*` tests (which did NOT exist before — added here), `bundle_open_round_trip_against_mock_cache`, and `parse_geometry_layer_parses_known_fixture`. `open_connection_from_bytes` is EXCLUDED — it's part of the SQLite VFS deferred to 006. The `FgbWriter` fixture can't run on wasm32 (it opens a tempfile; `env::temp_dir` panics with "no filesystem on this platform"), so `one_feature_fgb_bytes()` is target-split: host generates via the writer (preserving the writer→reader round trip), wasm32 loads the committed `shared/tests/samples/one-feature.fgb` (496 bytes, generated by the host-only `#[ignore]`d `dump_sample_fgb`). This reverses T038's no-committed-binary choice for the wasm path specifically, because the writer is unavailable there.
- [x] T046 Ran `wasm-pack test --headless --chrome` from `shared/` against locally-installed Chrome 149 + matching chromedriver 149. All 7 wasm tests pass headless. Per spec SC-002 + SC-004.
