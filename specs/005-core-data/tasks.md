...
- [ ] T001 Add `"core"` to the workspace `members` array in `/Users/singularity/eafora/Cargo.toml`.
- [ ] T002 Create `/Users/singularity/eafora/core/Cargo.toml` with `[package] name = "core", version = "0.0.0", edition.workspace = true, publish = false`; declare `[lib]` (no `[[bin]]`); add dependencies block referencing workspace pins: `serde`, `serde_json`, `chrono`, `uuid`, `sha2`, `rusqlite`, `flatgeobuf`, `geozero`, `geo-types`, `minimer`, `log`, `bytes`, plus `tokio = { workspace = true, default-features = false, features = ["sync"] }`. Add `[target.'cfg(target_arch = "wasm32")'.dependencies]` block with `wasm-bindgen`, `wasm-bindgen-futures`, `js-sys` (versions resolved at this step from `wasm-bindgen-test`'s required pin — verify at implementation time and add to root `[workspace.dependencies]`). Add `[dev-dependencies]` with `tempfile` and `tokio = { workspace = true, default-features = false, features = ["sync", "macros", "rt"] }` (test harness needs `macros` for `#[tokio::test]` and `rt` for the single-threaded runtime). Add `[target.'cfg(target_arch = "wasm32")'.dev-dependencies]` with `wasm-bindgen-test`.
- [ ] T003 [P] Add the wasm-bindgen-family pins to `[workspace.dependencies]` in `/Users/singularity/eafora/Cargo.toml`: `wasm-bindgen`, `wasm-bindgen-futures`, `js-sys`, `wasm-bindgen-test` (resolve versions against the latest stable wasm-bindgen and its required wasm-bindgen-test version at implementation time).
- [ ] T004 [P] Create `/Users/singularity/eafora/rust-toolchain.toml` pinning `channel = "1.83"` (or the most recent stable at implementation time; floor is 1.75 for stable AFIT per plan.md §Topic 1).
- [ ] T005 Create `/Users/singularity/eafora/core/src/lib.rs` containing only the `pub mod` declarations + the wildcard re-exports per data-model.md §Public-API surface summary (initially these point at empty modules; modules get bodies in subsequent phases). Run `cargo check --workspace` to confirm the empty-shell compiles.
...
- [ ] T006 [Foundational] Create `/Users/singularity/eafora/core/src/error.rs` with `minimer::define_app_error!(pub AppError);` plus `impl_from_error!` for: `serde_json::Error`, `rusqlite::Error`, `flatgeobuf::Error`, `geozero::error::GeozeroError`, `log::SetLoggerError`. Move `render_error_chain(error: &dyn Error) -> String` verbatim from `/Users/singularity/eafora/ingestion/src/error.rs`. Per plan §Outstanding decision #1, this is the canonical home; ingestion re-exports.
- [ ] T007 [Foundational] Rewrite `/Users/singularity/eafora/ingestion/src/error.rs` to `pub use core::error::{AppError, render_error_chain};` followed by the `impl_from_error!` calls for the ingestion-only error families that DON'T touch core: `sqlx::Error`, `reqwest::Error`, `zip::result::ZipError`, `shapefile::Error`, `shapefile::dbase::Error`, `secr::error::Error`, `dotenvy::Error`, `base64::DecodeError`. Verify the orphan rule allows these local-impl-for-local-deps (it does — local crate adding a `From` for a foreign type, but `AppError` is now `core::AppError` which is also foreign to ingestion; minimer's `impl_from_error!` macro should still work because the trait is `From<X> for AppError` where `AppError` is the local-to-the-defining-crate type. If the macro doesn't compile from this position, fall back to writing the impls by hand inside ingestion: `impl From<sqlx::Error> for AppError { fn from(...) -> Self { AppError::from(format!("{}", e)) } }`).
- [ ] T008 [Foundational] Add `core = { workspace = true }` to `/Users/singularity/eafora/ingestion/Cargo.toml`'s `[dependencies]` block.
- [ ] T009 [Foundational] Add `core = { path = "core" }` (or equivalent) to `/Users/singularity/eafora/Cargo.toml`'s `[workspace.dependencies]` block so other crates can reach `core` via `{ workspace = true }`.
- [ ] T010 [Foundational] Create `/Users/singularity/eafora/core/src/filesystem.rs` by moving the entire body of `/Users/singularity/eafora/ingestion/src/filesystem.rs`. Add `#[cfg(not(target_arch = "wasm32"))]` gates to `sha256_hex_of_file`, `filename_of`, `read_bytes`, `load_hashed_file`. Cross-target items (`FileReference`, `Hashed<T>`, `sha256_hex`) work on both. Add NEW function `verify_sha256(bytes: &[u8], expected_hex: &str) -> Result<(), AppError>` per spec FR-009: on mismatch, returns `AppError` whose message contains both `expected_hex` (first 8 hex chars) and the actual hash (first 8 hex chars).
...
- [ ] T015 [US1] Create `/Users/singularity/eafora/core/src/canonical/canonical_model.rs` by moving the enums from `/Users/singularity/eafora/ingestion/src/canonical/canonical_model.rs`: `StatisticKind`, `DataSourceKind`, `DataStatus`, `LicenseClass`, `LicenseShardClass`, `SourceRevision`. Add `Serialize` + `Deserialize` derives to the four enum types (StatisticKind, DataSourceKind, DataStatus, LicenseClass, LicenseShardClass) using `#[serde(try_from = "&str", into = "&str")]` (or the appropriate serde attribute that delegates to the existing `TryFrom<&str>` + `code()` / `as_str()` impls). `SourceRevision` already has Serialize / Deserialize; moves verbatim. The `Region` / `Country` / `Statistic` / `*Entity` / `*Projection` / `DataSource` / `StatisticValue` / `SourceChoice` types STAY in ingestion (they're producer-only; no consumer needs them at this layer).
- [ ] T016 [US1] Rewrite `/Users/singularity/eafora/ingestion/src/canonical/canonical_model.rs` to `pub use core::canonical::canonical_model::*;` at the top, followed by the remaining ingestion-only struct definitions (`Region`, `RegionEntity`, `Country`, `CountryEntity`, `Statistic`, `StatisticEntity`, `DataSource`, `DataSourceEntity`, `StatisticValue`, `StatisticValueEntity`, `SourceChoice`, `SourceChoiceEntity`) and their `From` / `TryFrom` impls. Per `feedback_wildcard_re_exports`: the wildcard re-export covers the moved types; existing `crate::canonical::canonical_model::StatisticKind` call sites resolve through it.
...
- [ ] T031 [US4] Implement `/Users/singularity/eafora/core/src/artifact/discovery.rs` per data-model.md §Module: `core::artifact::discovery`. Define `DISCOVERY_SCHEMA_VERSION = 1`, `DiscoveryDocument` struct with `#[derive(Debug, Clone, Serialize, Deserialize)]`, fields `schema_version: u32`, `repository_base_url: String`, `minimum_client_version: String`, `sunset: Option<String>`. Implement `parse_discovery_document(bytes: &[u8]) -> Result<DiscoveryDocument, AppError>` per spec FR-015: serde-deserialize, validate `schema_version == DISCOVERY_SCHEMA_VERSION` (reject others with the documented `AppError` shape). Make T030 pass.
...
- [ ] T036 [US5] Create `/Users/singularity/eafora/core/src/sqlite/mod.rs` with `pub mod vfs; pub use vfs::*;`.
- [ ] T037 [US5] Implement `/Users/singularity/eafora/core/src/sqlite/vfs.rs` per data-model.md §Module: `core::sqlite::vfs`. Public surface: `register_vec_u8_vfs() -> Result<(), AppError>` (wasm32 only, `#[cfg(target_arch = "wasm32")]`), `open_connection_from_bytes(name: &str, bytes: Vec<u8>) -> Result<rusqlite::Connection, AppError>` (cross-target). On native: implementation uses rusqlite's `Connection::deserialize` API to load bytes directly into an in-memory database. On wasm32: implementation registers a custom VFS that reads pages from a `Vec<u8>` held in the connection's open-state user-data, then opens the connection against that VFS. Implementation details are plan-time verified per research Topic 2; if `rusqlite` bundled SQLite doesn't cross-compile cleanly to `wasm32-unknown-unknown`, fall back to `sqlite-wasm-rs` per the research fallback (note: this would require updating `core/Cargo.toml`'s wasm32 deps + the architecture doc). Make T035 pass on both targets.
...
- [ ] T039 [US5] Implement `/Users/singularity/eafora/core/src/artifact/geometry.rs` per data-model.md §Module: `core::artifact::geometry`. Define `Feature`, `Polygon`, `BoundingBox` structs per the data-model spec. Implement `FlatGeobufReader` wrapping `flatgeobuf::FgbReader<std::io::Cursor<Vec<u8>>>`; expose `iter_features() -> impl Iterator<Item = Result<Feature, AppError>>` and `features_in_bbox(bbox: BoundingBox) -> impl Iterator<Item = Result<Feature, AppError>>` (the bbox-query exercises the R-tree spatial index that 006's hit-test path consumes). Implement `open_flatgeobuf_reader(bytes: Vec<u8>) -> Result<FlatGeobufReader, AppError>` — parse the bytes into the upstream reader eagerly; any parse error returns `AppError`. Make T038 pass.
  ...
  Phase 3 (US1, T014-T019)   ← ready after Phase 2
       ...
       Phase 4 (US2, T020-T025)   ← needs Phase 3 (the moved types)
            ...
            Phase 7 (US6, T032-T034)         ↙
                  ↓                         ↙
                  Phase 8 (US5, T035-T043)  ← needs US2 + US3 + US6 (and the geometry / VFS internal sub-phase)
                        ↓
...
- **Phase 3 (US1)**: T014 (mod.rs) and T015 (canonical_model.rs) touch different files — drafted in parallel; T016 (ingestion rewrite) depends on T015; T017 (StatisticShardKey move) is independent of the canonical-model move.
...
- **Phase 8 (US5)**: T035-T037 (SQLite VFS) and T038-T039 (geometry reader) touch different files — fully parallel sub-phases; both must complete before T040-T043 (bundle).
- **Phase 10**: T047, T048, T049, T050, T051, T052 are all parallelizable (different commands, mostly read-only).

## Implementation strategy

Single PR per plan.md §Phasing. Total task count: 53. Estimated effort: ~2 days of focused work (the type extraction is mechanical; the WASM VFS + geometry-reader fixtures are the time sinks). The MVP-shaped subset is US1 + US2 (Phases 1-4) — that's enough to verify the workspace member compiles + types extracted cleanly + manifest parses end-to-end. US3 + US4 + US6 are small (~3 tasks each); US5 (Bundle::open) is the largest single chunk because it depends on the SQLite VFS + the geometry reader + every prior story.
