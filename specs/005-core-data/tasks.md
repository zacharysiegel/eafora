...
- [ ] T015b [P] [US1] In `/Users/singularity/eafora/ingestion/src/adapter/adapter_model.rs`, remove the local `NaiveDatePeriod` struct + impl block. Migrate every `crate::adapter::NaiveDatePeriod` / `crate::adapter::adapter_model::NaiveDatePeriod` call site across ingestion to `shared::canonical::canonical_model::NaiveDatePeriod` directly (no re-export). The other `adapter_model.rs` types (`AdapterOptions`, `NormalizedStatisticValue`, `NormalizeOutcome`, `IngestWarning`, `IngestWarningKind`) stay verbatim — they're producer-only normalization-pipeline types.
- [ ] T016 [US1] Rewrite `/Users/singularity/eafora/ingestion/src/canonical/canonical_model.rs` to hold ONLY the producer-side types: the `*Entity` definitions (`RegionEntity`, `CountryEntity`, `StatisticEntity`, `DataSourceEntity`, `StatisticValueEntity`, `SourceChoiceEntity`) and their `(Try)From<Entity> for Model` impls (kept per T015a), plus the producer-only Models `StatisticValue` + `SourceChoice` (consumer never sees them per spec FR-005). NO `pub use shared::...` re-export. Migrate every ingestion call site that referenced a moved type (`crate::canonical::canonical_model::StatisticKind`, `::Region`, `::Country`, `::Statistic`, `::DataSource`, the moved enums, `NaiveDatePeriod`) to the corresponding `shared::canonical::canonical_model::*` path directly. **Rename**: the file/module name `canonical_model` becomes a misnomer once the canonical Models leave (it now holds Entities + conversions + producer-only Models); rename it to `canonical_entity.rs` (proposed) as part of this task — update the `pub mod` line in `ingestion/src/canonical/mod.rs` and the migrated call-site paths accordingly.
- [ ] T017 [US1] Move `StatisticShardKey` from `/Users/singularity/eafora/ingestion/src/artifact/artifact_model.rs` to `/Users/singularity/eafora/shared/src/artifact/bundle.rs` (the file will be created in T031; this task adds the type at the head of that file with `#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize)]`). Migrate ingestion's `StatisticShardKey` call sites to `shared::artifact::bundle::StatisticShardKey` directly (no re-export). Required for FR-018 + the bundle's `shard_bytes: BTreeMap<StatisticShardKey, Vec<u8>>` field.
