...
> §Scope cutoff explicitly names what's NOT in this feature (live CDN fetch against real `repository.eafora.org`, SSG content surfaces beyond in-app sheets, `shared/`-change watcher, design-token codegen, App Intents, push / iCloud / WidgetKit, Android). Each item is paired with the architectural reference that documents WHY it's out of scope and the trigger / follow-up that would pull it in.

- [x] Dependencies and assumptions identified

> §Assumptions names: (a) the `shared` crate's consumer surface, which has since landed on `master`, though its FFI boundary has not and is this feature's first phase; (b) `ingestion build --downsampled` prerequisite (same as web); (c) `repository.eafora.org` is NOT a prerequisite (same stub-shaped fallback path web uses); (d) Apple Developer Program enrollment, which does not exist yet and gates P4 only; (e) the build host, which has Xcode 26.5 but no simulator runtime, no iOS Rust targets, and no xcodegen; (f) `uniffi-bindgen-swift` per-artifact invocation pattern; (g) wgpu Metal-from-`CAMetalLayer` API stability; (h) XcodeGen YAML schema stability; (i) `MTKView.isPaused` + `setNeedsDisplay()` semantics on iOS 18+; (j) `xcodebuild -exportArchive` direct-upload behavior; (k) AASA Worker route precedence; (l) Cloudflare-registered domain.
