...
- **They add a `unit` dimension** the country-level dataset has none of, with members `NR` and `YR`. The unit is determined by the indicator and the two do not overlap: at NUTS-1 every one of the 3,484 `TOTFERRT` observations sits under `NR` and every one of the 3,483 `AGEMOTH` observations under `YR`. Pinning the unit is therefore wrong, and wrong silently, dropping mean age entirely; the request omits it and the parser projects the axis away as it already does `freq`. That projection is only sound while it stays injective, which the parser asserts rather than assumes.
...
Fully specified. The design is `docs/architecture/geometry.md`; the task breakdown, the facts measured from the boundary distribution, and every settled decision are in [tasks.md](tasks.md) §Phase D. What follows is only the evidence gathered here that those documents cite rather than restate.

The sketch this section used to hold was written from a WFS probe, before the 507 MB distribution was in hand. The file disagrees with the probe on almost every count, so nothing from the probe should be relied on: it reported 16,114 features across 35 countries and 1,437 distinct codes, where the distribution holds 16,899 parts across 34 countries and 1,411 codes, and Serbia is absent from it as well as Montenegro and Türkiye. The probe's one durable finding is that there is no NUTS-2 layer, so the coarser levels are derived by unioning NUTS-3 on the code's prefix.

**Türkiye's provinces were verified against Eurostat's regions, and that evidence still stands.** Natural Earth's admin-1 layer carries exactly 81 Turkish provinces against Eurostat's 81 Turkish NUTS-3 regions, matching one to one with nothing left over on either side, which follows from Turkish NUTS-3 being defined on the provinces themselves; grouping them on the four-character prefix yields exactly the 26 NUTS-2 regions Eurostat publishes. The two sources name a province with unrelated identifiers, Natural Earth carrying ISO 3166-2 (`TR-34`) and Eurostat a NUTS code (`TR100`), with no computable relation between them, which is why Phase C0 put both on the region: the pair is seed data rather than a mapping the pipeline consults. The names were matched once, to verify the pairing; the build joins on the identifier.
...
What this does not establish is that Natural Earth's provinces align with EuroGlobalMap's NUTS-3 polygons along the Turkish border, where the two sources meet. The subtraction in `geometry.md` handles that seam the same way it handles every other one.

Two claims this section carried have since been answered rather than deferred. The committed decision in `docs/architecture/ingestion.md` §Geometry ingestion naming a Natural Earth provinces layer for subnational work is correct only where provinces and NUTS coincide, which is Türkiye; elsewhere provinces do not nest with NUTS and cannot carry NUTS values, so EuroGlobalMap serves those countries. And the licence, called a gating question here, is settled in `geometry.md` §Licence obligations.

Feature volume remains measurable rather than known: the current world layer is one feature per country at 1:50m, against a layer holding every level. `./scripts/build/measure-site-budget.sh` reports it against the first-paint and second-paint caps once a file exists.
