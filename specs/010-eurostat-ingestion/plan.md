...
- **`docs/architecture/geometry.md` is not on `master` yet.** The layered-geometry design, including the boundary-source decision and the subtraction that reconciles two sources, is written and awaiting integration on `geometry-layers-design`. Until it merges, the only committed subnational geometry decision is `docs/architecture/ingestion.md` §Geometry ingestion naming `ne_10m_admin_1_states_provinces.zip`, which is provinces rather than NUTS regions and does not nest with them. Phase D depends on that document, so this feature's Phase D sketch points at it and Phase D cannot start before it lands.
