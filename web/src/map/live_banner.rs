...
pub fn LiveBanner() -> impl IntoView {
    ...
    Effect::new(move |_| {
        if !live_notice_shown.get() {
            return;
        }

        set_timeout(move || live_notice_shown.set(false), VISIBLE_DURATION);
    });

    view! {
        <Show when=move || live_notice_shown.get()>
            <div class="map-live-banner panel" role="status">
                ...
                <button
                    ...
                    on:click=move |_| live_notice_shown.set(false)
                >
