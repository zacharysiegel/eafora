...
pub fn LiveBanner() -> impl IntoView {
    ...
    Effect::new(move |_| {
        if !live_load_failed.get() {
            return;
        }

        set_timeout(move || live_load_failed.set(false), VISIBLE_DURATION);
    });

    view! {
        <Show when=move || live_load_failed.get()>
            <div class="map-live-banner panel" role="status">
                ...
                <button
                    ...
                    on:click=move |_| live_load_failed.set(false)
                >
