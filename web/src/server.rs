...
/* Renders with the async mode rather than either streaming mode so the result is one complete document;
   ...
   scripts. */
pub async fn render_shell_document(leptos_options: LeptosOptions) -> Bytes {
...
/* Also initializes the global executor that rendering spawns onto, which the renderer panics without
   and which nothing else on this path sets up. Reporting the app's declared routes makes a route that
   this export does not write fail here rather than reach a deploy undeployed. */
...
pub fn read_manifest_options() -> LeptosOptions {
