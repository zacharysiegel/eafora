...
pub async fn load_embedded_bundle(cache: &OpfsArtifactCache) -> Result<Bundle, AppError> {
    ...
    let manifest_bytes: Vec<u8> = fetch::fetch_bytes(&HttpRequest { method: HttpMethod::Get, url: manifest_url }).await?;
    ...
    for entry in manifest.file_entries() {
        ...
        let file_bytes: Vec<u8> = fetch::fetch_bytes(&HttpRequest { method: HttpMethod::Get, url: file_url }).await?;
