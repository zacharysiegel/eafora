...
pub(crate) mod tests {
    ...
    // The upstream FgbWriter opens a tempfile (env::temp_dir), which panics on wasm32 ("no
    // filesystem on this platform"). So the fixture is generated by the writer on the host and
    // committed as tests/samples/one-feature.fgb; the wasm test build loads those bytes instead.
    ...
    fn dump_sample_fgb() {
        std::fs::write(
            concat!(env!("CARGO_MANIFEST_DIR"), "/tests/samples/one-feature.fgb"),
            one_feature_fgb_bytes(),
        )
        .unwrap();
    }

    /// One-feature FlatGeobuf (a rectangle for "TST" / "Testland"). `pub(crate)` so `bundle.rs`'s
    /// tests reuse it. On the host it is generated by the upstream `FgbWriter` (exercises the
    /// writer -> reader round trip); on wasm32 the writer can't run (no filesystem for its
    /// tempfile), so the committed sample is loaded instead.
    #[cfg(not(target_arch = "wasm32"))]
