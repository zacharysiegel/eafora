...
// COUNTRY_STATE_ARRAY_LEN on the Rust side; the bind group's min_binding_size check catches any drift.
// Padded to 16 bytes so the uniform-array element size is a multiple of 16 (stricter WGSL validators
// such as WebKit require this); matches the Rust CountryState struct.
struct CountryState {
    lift_px: f32,
    outline_px: f32,
    padding0: f32,
