...
/// origin, summed over the ring and halved. Only its sign is used here: positive if the vertices are
/// ordered counterclockwise around the interior, negative if clockwise; `edge_outward_normal` uses that
/// to rotate each edge toward the outside.
