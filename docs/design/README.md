...
- Region fills use a continuous single-hue scale — a white-to-red lerp over the statistic's value range, not discrete buckets. No multi-hue choropleths; the data itself is the only saturated color on the map. For TFR the most-saturated red marks the lowest (below-replacement) value and white the highest; no-data regions are white too.
