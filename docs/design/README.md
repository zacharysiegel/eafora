...
The second is the detail dock's scroll thumb: nothing at rest, two hairlines wide while the pointer is in the panel, seven pixels and an opaque sheet behind one hairline while the pointer is on it or holding it. It is included because the mark exists to be grabbed and a thing that appears at full size under the cursor reads as a glitch rather than as an affordance. Its width and its fill are what move, and only on the way back: arriving, the sheet appears at once. The width shrinks to exactly two hairlines, so the borders meet and squeeze out the white they enclosed, while the fill darkens on a curve that is nearly flat at first; the two finish together, which is what keeps the last sliver of white from being erased in one frame. The thumb's position along the track still cuts instantly to wherever the content is.
