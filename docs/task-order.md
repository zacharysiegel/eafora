...
1. **`003-web-client`** — the browser client (WASM + Leptos shell + map view + OPFS cache + perf-budget). Planning artifacts merged; delivered in phases, full breakdown in `specs/003-web-client/plan.md` §Phasing for PRs. **In progress.**
   ...
   - Phase E (perf-budget script + static shell export + `wrangler` deploy config) — **In progress** (`phase-e-deploy-and-budget`). Landed: `scripts/build/measure-site-budget.sh`, `scripts/build/precompress-site.sh`, `scripts/build/build-site.sh`, `scripts/build/deploy-site.sh`, `web/wrangler.toml`, `web/static/_headers`, `web/static/.assetsignore`, the `web export-shell` mode that writes the static document the deploy serves at `/`, and the `recursion_limit` raise both crate roots needed to compile a release build at all. Content hashing is enabled per-build via `LEPTOS_HASH_FILES=true` rather than in the manifest, because cargo-leptos only re-hashes on a full build and a `watch` rebuild would otherwise serve stale assets. Two things remain, both waiting on one probe deploy: whether Cloudflare Workers Assets serves uploaded `.br` siblings, which decides whether `precompress-site.sh` survives and what §Compression and §Why Workers Assets instead of Cloudflare Pages in `client-web.md` should say; and confirmation of the `Content-Type` and 404 behavior `_headers` and `wrangler.toml` were written against. The perf-budget check runs from `scripts/git/pr-integrate.sh` rather than CI, since no hosted compute is paid for yet.
