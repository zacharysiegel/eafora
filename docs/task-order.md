...
1. **`003-web-client`** — the browser client (WASM + Leptos shell + map view + OPFS cache + perf-budget). Planning artifacts merged; delivered in phases, full breakdown in `specs/003-web-client/plan.md` §Phasing for PRs. **In progress.**
   ...
   - Phase E (perf-budget script + precompress + `wrangler` deploy config) — **In progress** (`phase-e-deploy-and-budget`). `scripts/measure-site-budget.sh` (FR-006) and `scripts/precompress-site.sh` (FR-005) have landed, along with the `recursion_limit` raise that release builds needed in order to compile at all. `wrangler.toml` (FR-032) and `web/static/_headers` (FR-033) are deliberately not written yet: a static-only Workers Assets deploy cannot serve the server-rendered page shell, and FR-033's long-cache rule assumes content-hashed asset filenames that cargo-leptos is not currently producing. Both need a decision before the config can say anything correct. FR-006's CI invocation is also unbuilt, since no CI exists and `docs/architecture/overview.md` §CI/CD defers the tool choice.
