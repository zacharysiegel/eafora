...
1. **`003-web-client`** — the browser client (WASM + Leptos shell + map view + OPFS cache + perf-budget). Planning artifacts merged; delivered in phases, full breakdown in `specs/003-web-client/plan.md` §Phasing for PRs. **In progress.**
   ...
   - Phase E (perf-budget report + build-time prerender + `wrangler` deploy config) — landed. `web prerender` renders `/` once and writes the document the static deploy serves, since production runs no server; `build-site.sh` orders the build before the export because a build empties the site root, and `verify-site-tree.sh` refuses a tree whose document references a missing or stable-named asset. Content hashing is per-build via `LEPTOS_HASH_FILES=true` rather than in the manifest, because cargo-leptos re-hashes only on a full build and a `watch` rebuild would otherwise serve stale assets. The perf-budget targets bound artifact bytes (2 MB first paint, 8 MB second paint) with client code reported uncapped, and the report runs from `scripts/git/pr-integrate.sh` rather than CI, since no hosted compute is paid for yet.
     - A probe deploy settled the deploy behavior the config was written against: `/` resolves to `index.html` with no `not_found_handling` set, a path with no asset returns 404 with an empty body, `_headers` is applied, and a `.sqlite` with no rule is served with no `Content-Type`, which is why `_headers` states one.
     - The same probe refuted precompression, so `precompress-site.sh` is deleted and spec FR-005 is withdrawn: uploaded `.br` siblings are not negotiated and each is served as its own asset with neither `Content-Type` nor `Content-Encoding`. It also confirmed the artifacts transfer whole, since Cloudflare's auto-compress content types include no generic binary type. Recovering that approx. 3.07 MB now depends on compressing artifacts in the producer, tracked in `docs/backlog.md`.
