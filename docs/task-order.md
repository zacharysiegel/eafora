...
2. **`ingestion build --downsampled`** — producer command that emits the downsampled embedded bundle (web-client Phase 0b prerequisite; unblocks `scripts/sync-embedded-bundle.sh` and the real embedded/live bundle). Off `master`; independent of the web stack, so it can land any time before Phase E (an interim hand-stub covers Phase C). **Pending.**
