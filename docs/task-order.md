...
2. **The year scrubber's capsule overshoots the rail under browser zoom** — the capsule leaves the rail at a non-default zoom and lands on the upper bound label, though it is flush at both extremes at 100%. Cause undiagnosed; the candidates and the measurement to make are in `docs/backlog.md` §Client. Ordered here at the owner's direction, before the iOS client.
3. **The rank reads "1st lowest of 217"** — it should read "Lowest of 217", and the other extreme should probably read "Highest of 217" rather than "217th lowest of 217". Detail in `docs/backlog.md` §Client. Ordered here at the owner's direction, before the iOS client.
4. **`004-ios-client`** — the iOS surface: a UniFFI boundary, a SwiftUI shell, and the Metal surface. Spec, plan, and tasks in `specs/004-ios-client/`. **In progress** (`004-ios-client`), planning only so far.
