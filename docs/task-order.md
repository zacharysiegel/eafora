...
2. **Schema version backtracking** — a published manifest at a schema version a deployed client cannot read currently stops that client updating entirely, because the live path fetches only `latest/manifest.json`. Publishing an index of versions and having the client open the newest one it understands removes the deploy-order constraint that a schema bump imposes today. Detail in `docs/backlog.md` §Client. Ordered here at the owner's direction, immediately after the region detail dock.
