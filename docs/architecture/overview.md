...
- **Hostnames**: the app is served from `app.eafora.org`, which is the hostname `shared/src/artifact/discovery.rs` commits every client to permanently, and artifacts from `repository.eafora.org`. The apex and `www` are Cloudflare Redirect Rules pointing at `app.eafora.org`. They are not a second copy of the app: browser storage is origin-scoped, so a reader who arrived at a second hostname would re-download the whole bundle and build a separate OPFS cache (see `docs/architecture/client-web.md` §OPFS cache adapter), which means the app has exactly one origin and everything else redirects to it. Keeping the app off the apex also leaves the apex free to serve something with COOP and COEP headers later without closing the app origin to embedding (`docs/education/web-shared-array-buffer.md` §5. The trade for Eafora).
- **No landing page in v1**: the apex redirects rather than presenting a wordmark, a subtitle, and a button. `docs/design/README.md` §Naming and the About page fixes the principle that the reader lands on the data with no splash screen, and a page in front of the app is that splash screen one hostname over; a link shared in a chat or an article would gain a hop, and a slogan is where the mission's editorial neutrality is hardest to hold. The name's etymology already has a designed home on the About surface. A landing page earns the apex when it has content the app should not carry: store links once the mobile apps ship, press material, partner and funder pages, or writing. Until then those live as static routes on the app origin, which §Routing and SSG already provides for.
