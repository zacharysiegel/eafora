...
2. **Animations API.** Reuse Singularity's `LockedSwitch`-style state-machine pattern in `core::geometry::animation`, or invent something new? Singularity's pattern is fine for stage transitions but the camera animation here is continuous, not discrete; probably a different shape.
...
4. **Live-API readiness for v3.** The `ingestion/` actix-web binary is provisioned for a future live API but not wired. The most likely v3 use case is user-submitted corrections (Wikipedia-style) — auth, moderation, schemas, and rate-limiting all need their own spec when the time comes. **Semantic / NL-Q&A-style features are explicitly not on the roadmap.**
