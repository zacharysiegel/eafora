...
- **Statistics: include the headline statistic (`tfr`) at full historical depth**; exclude all other statistics from the embedded bundle. Approx. 2.0 MB.

Total embedded bundle: approx. 3.5 MB. The intent is that TFR — the centerpiece indicator — is fully usable offline at the same visual fidelity as the online experience, while supplementary statistics simply aren't visible until the CDN fetch lands. Matching the live bundle's geometry resolution avoids a low-res-then-high-res transition when the swap happens.

Revisit the embedded-statistic set when v2+ subnational TFR data lands (might push the bundle past acceptable native-binary size) or when a second indicator becomes prominent enough to also warrant the offline floor.
