...
The client identifies its **distribution context** at startup (serving eafora.org itself, or running inside a third party's site) and looks up the *authorized class set* — the subset of license classes its context is permitted to access. The context is a property of how the client is distributed, never of which artifact bundle it happens to have loaded. For v1 every context authorizes `base` and there is nothing else to choose between; the mechanism is exercised trivially.
