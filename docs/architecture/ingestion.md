...
- **Pipeline**: Natural Earth is fetched at artifact-build time from a pinned URL (`https://naciscdn.org/naturalearth/...`) and cached locally on the build machine under a gitignored path (e.g. `target/natural-earth-cache/<pinned-version>/`). The geometry is processed in-memory: project to WGS84 (already is), join to `country.iso3` via Natural Earth's `ADM0_A3` field, emit as FlatGeobuf with the R-tree spatial index built in, write to the artifact output directory. **Nothing is staged into Postgres; nothing is checked into git.** The local cache is purely a build-speed optimization — first build downloads from the CDN, subsequent builds reuse the cached zip. If the CDN is unreachable and the cache is empty, the build fails (rare combined failure mode).
...
- **`emit_geometry_flatgeobuf(output_dir)`** → `ShardOutput`. Ensures the pinned Natural Earth release is cached locally (downloads from `naciscdn.org` if not), processes the shapefile in-memory to the FlatGeobuf shape we ship (joined to `country.iso3` by `ADM0_A3`), writes to `output_dir/geometry/`. Single file, no license-class split — Natural Earth is `public_domain`.
