...
- **Features matching no seeded country**: dropped. Natural Earth ships four such features in the pinned 50m release: `ATA` (Antarctica), `ATC` (Ashmore and Cartier Is.), `IOA` (Indian Ocean Ter.), and `KAS` (Siachen Glacier). Dropping `KAS` leaves the only gap visible at default zoom, a wedge of bare substrate near Kashmir measuring approx. 50 by 80 km, between the Indian, Pakistani and Chinese fills; a grid scan at 0.05 degrees over latitude 30 to 40 and longitude 70 to 82 found no other uncovered point. The gap is accepted. Filling it means either emitting it under a region code no statistic matches, which paints a distinct tone and so asserts a distinguishable region the source does not recognize, or folding it into a neighbouring country, which moves the drawn boundary onto a line that is not settled (Google Maps draws no line at all across this stretch). Neither is worth asserting for land with no population and no statistic.
